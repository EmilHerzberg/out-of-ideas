...
After 1,856 generations across 10 frontier LLMs (each with ≥10 generation attempts in every one of 10 archetype cells) and 24 orchestrator runs, the numbers are clear:
...
### Finding 4 — default-repertoire depth is measurable per (model × cell)

The matrix in `IDEAS_MATRIX.md` shows the survival-rate-as-a-proxy for default-repertoire depth across all 100 (model × archetype) cells in our headline dataset (10 models × 10 archetypes, every cell ≥10 attempts). Sample:

```
                  cause_effect   misconception   odd_one_out   estimation
DeepSeek V4-Pro       22%            41%            25%           40%
Claude Opus 4.7        8%            33%            22%           50%
Gemini 2.5 Pro        38%            16%            33%            4%
OpenAI GPT-5          41%            16%            50%           60%
Qwen 3.6 Max          38%            70%            72%           43%
```

Read this as: "Of every 100 questions generated by Qwen 3.6 Max in the `odd_one_out` cell, 72 are distinct enough to ship." Or: "Anthropic Opus 4.7's default-repertoire depth for `cause_effect` is roughly 8% of generations — it paraphrases very aggressively in that cell."
...
- **`IDEAS_MATRIX.md`** — six matrices over the 10-model × 10-archetype headline scope: generation attempts (A), survival rate (B), $/unique idea (C), combined view (D), quality scores per cell (E, proves "unique" ≠ "low quality"), and saturation decay over time (F, proves "running out of ideas" is a real temporal phenomenon).
- **`ANALYSIS.md`** — full benchmark report with $/unique-idea numbers, saturation curves, methodology.

Both reproduce from `data/finalized-pool.jsonl` and `data/auto-runs/*/run.log.jsonl`. Run `node analyze-all.mjs` to verify.

---

*Project: out-of-ideas. 586 in-scope questions, 24 orchestrator runs, 10 frontier LLMs (each ≥10 attempts in every archetype), $69 total spend. May 2026.*
